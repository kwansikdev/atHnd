...
import { SearchFigureDto } from "../model";

type MyFigureAddFormStore = {
  figures: SearchFigureDto[];
  selectedFigures: SearchFigureDto[];
  // figureShop: Database["public"]["Tables"]["figure_shop"]["Row"][];
  figureShop: Array<{ value: string; label: string; parentId?: string }>;
  setFigures: (
    figures:
      | SearchFigureDto[]
      | ((prev: SearchFigureDto[]) => SearchFigureDto[]),
  ) => void;
  setSelectedFigures: (figure: SearchFigureDto) => void;
  setFigureShop: (
    shop: Array<{ value: string; label: string; parentId?: string }>,
  ) => void;
  addFigures: (newFigures: SearchFigureDto[]) => void;
  addSelectedFigure: (figure: SearchFigureDto) => void; // 추가
  removeSelectedFigure: (figureId: string) => void; // 추가
  toggleSelectedFigure: (figure: SearchFigureDto) => void; // 추가
  reset: () => void;
};

export const useMyFigureAddFormStore = create<MyFigureAddFormStore>((set) => ({
  figures: [],
  selectedFigures: [],
  figureShop: [],
